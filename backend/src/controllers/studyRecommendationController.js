...
export const getStudyRecommendations = async (req, res) => {
  try {
    ...
    // Get quiz titles for context
    const quizIds = attempts.map(a => a.quizId);
    const quizzes = await TeacherQuiz.find({ _id: { $in: quizIds } }).select('title subject');
    const quizMap = {};
    quizzes.forEach(q => { quizMap[q._id.toString()] = { title: q.title, subject: q.subject }; });
    ...
    const avgScore = attempts.reduce((sum, a) => sum + (a.finalScore || 0), 0) / attempts.length;
    const totalHints = attempts.reduce((sum, a) => sum + (a.hintsUsed || 0), 0);
    const lowScoreAttempts = attempts.filter(a => (a.finalScore || 0) < 50);
    const confusedCount = attempts.reduce((sum, a) => sum + (a.emotionalSummary?.confusedCount || 0), 0);

    // Build recent quiz context
...
${Object.entries(emotionCounts).map(([e, c]) => `- ${e}: ${Math.round((c/totalEmotions)*100)}%`).join('\n')}
